...
export const metacognitive: Architecture = {
  ...
  color: "#ef4444",
  paradigm: "bdi",
  conceptualInsight: "Knowing what you don't know is a capability. The self-model (knowledge_domain, available_tools, confidence_threshold) is a first-class data structure that the agent reads before every query. This makes the agent's reasoning about its own limitations explicit, inspectable, and tunable — rather than leaving it implicit in the prompt.",
  whenToUse: [
    { useCase: "Safety-critical domains where wrong answers cause harm", reason: "Explicit confidence scoring and a hard escalation threshold ensure the agent never answers beyond its verified capability — the 'when in doubt, escalate' rule is structural, not prompt-based." },
    { useCase: "Systems serving diverse query complexity levels", reason: "The three-way routing (reason/tool/escalate) matches resource expenditure to actual need — simple queries need no tool, complex ones use tools, ambiguous ones escalate." },
    { useCase: "Multi-agent systems where routing accuracy matters", reason: "The self-model makes the routing decision transparent and auditable — you can see exactly what confidence score triggered escalation and why." },
  ],
  strengths: [
    "Principled escalation based on an explicit confidence score",
    "Self-model is inspectable and tunable (not buried in prompt engineering)",
    "Three-way routing matches resource use to task complexity",
    "Safe by default: low confidence always routes to human",
  ],
  weaknesses: [
    "Self-model must be manually defined and maintained",
    "Confidence calibration quality determines routing accuracy",
    "No mechanism to update the self-model from experience",
    "Adds one full LLM call overhead before every task",
  ],
  comparedTo: [
    { name: "Dry-Run Harness", insight: "Dry-Run always routes to a human gate unconditionally; Metacognitive routes to a human only when confidence falls below threshold — most queries never reach a human." },
    { name: "Meta-Controller", insight: "Meta-Controller routes to the best specialist agent; Metacognitive routes based on its own capability gap — to a tool, or to a human when no tool suffices." },
  ],
  nodes: [
    { id: "start",     type: "start",         label: "START",               x: 250, y: 20 },
    { id: "analyze",   type: "a2decision",    label: "Metacognitive\nAnalysis",x: 250, y: 150, description: "A2 (Decision): reads self-model against query; produces strategy + confidence score" },
    { id: "reason",    type: "a2decision",    label: "Reason\nDirectly",    x: 80,  y: 310, description: "A2 (Decision): high confidence (>0.7) — answer from knowledge" },
    { id: "call_tool", type: "a5output",      label: "Call\nTool",          x: 250, y: 310, description: "A5 (Output): medium confidence (0.5–0.7) — execute a specialized tool" },
    { id: "escalate",  type: "a4coordination",label: "Escalate\nto Human",  x: 430, y: 310, description: "A4 (Coordination): low confidence (<0.5) — route to human expert" },
    { id: "synthesize",type: "a2decision",    label: "Synthesize\nResult",  x: 250, y: 450, description: "A2 (Decision): converts raw tool output into a patient-facing response" },
