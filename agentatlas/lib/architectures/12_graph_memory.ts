...
export const graphMemory: Architecture = {
  ...
  a1a5Profile: {
    a1input: "User query + document corpus — natural language question over a pre-ingested knowledge base",
    a2decision: "LLM graph maker → LLM Cypher generator → LLM synthesizer (three sequential LLM roles)",
    a3memory: "Knowledge graph in Neo4j (entity-relation triples extracted from all ingested documents)",
    a4coordination: "None — single-agent pipeline; no inter-agent routing",
    a5output: "Natural language answer derived from multi-hop graph traversal results",
  },
  nodes: [
    { id: "start",     type: "start",  label: "START",              x: 200, y: 20 },
    { id: "ingest",    type: "llm",    label: "Graph\nMaker Agent", x: 200, y: 160, description: "LLM: parses documents and extracts entity-relation triples into Neo4j" },
    { id: "cypher_gen",type: "llm",    label: "Cypher\nGenerator",  x: 200, y: 300, description: "LLM: translates natural language query into a Cypher graph query" },
    { id: "execute",   type: "memory", label: "Neo4j\nTraversal",   x: 440, y: 300, description: "Memory: executes Cypher against the knowledge graph, returns raw traversal results" },
    { id: "synthesize",type: "llm",    label: "Answer\nSynthesizer", x: 200, y: 440, description: "LLM: converts raw Cypher results into a natural language answer" },
